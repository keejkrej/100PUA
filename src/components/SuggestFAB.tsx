...
import { submitSuggestionMutation } from "~/lib/api-client";
...
export function SuggestFAB({ mode, topicSlug = "", topicTitle = "" }: Props) {
  const runSubmit = useAtomSet(submitSuggestionMutation, { mode: "promise" });
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (mode === "prompt" && !("promptBody" in apiPayload && apiPayload.promptBody)) return;
    ...
    try {
      ...
      const cause = err as { error?: string; message?: string };
      if (cause && typeof cause === "object" && cause.error === "misconfigured_server") {
        openGithubDraft(form);
        return;
      }
      if (cause && typeof cause === "object" && typeof cause.error === "string") {
        const msg =
  ...
  return (
    <>
      <Button
        ...
        aria-label={mode === "topic" ? "Suggest a new topic" : "Suggest a new prompt"}
        onClick={openDialog}
      >
        add
      </Button>
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogPopup id="suggest-dialog">
          <form className="flex flex-col gap-4" onSubmit={onSubmit}>
            ...
            {showFields ? (
              <DialogDescription>
                Your suggestion is turned into a GitHub issue when the API is configured; otherwise
                this opens a GitHub draft.
              </DialogDescription>
            ) : null}
            {feedback ? (
              <p
