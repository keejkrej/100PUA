const V = {},
  b = "data-astro-transition-persist",
  oe = ["data-astro-transition", "data-astro-transition-fallback"],
  N = new Set();
function W(e) {
  const t = e.src ? new URL(e.src, location.href).href : e.textContent;
  return N.has(t) ? !0 : (N.add(t), !1);
}
function re(e) {
  for (const t of e.scripts)
    !t.hasAttribute("data-astro-rerun") && W(t) && (t.dataset.astroExec = "");
}
function ie(e) {
  const t = document.documentElement,
    n = [...t.attributes].filter(
      ({ name: o }) => (t.removeAttribute(o), oe.includes(o)),
    );
...
function ae(e, t) {
  const n = [],
    ...
    r =
      typeof o.moveBefore == "function"
        ? (i, s, l) => i.moveBefore(s, l)
        : null;
  for (const i of t.querySelectorAll(`[${b}]`)) {
    const s = i.getAttribute(b),
      l = e.querySelector(`[${b}="${s}"]`);
    l &&
      (n.push({ old: i, newTarget: l }), r ? r(o, i, null) : o.appendChild(i));
  }
  t.replaceWith(e);
  for (const { old: i, newTarget: s } of n)
    (r ? (r(s.parentNode, i, s), s.remove()) : s.replaceWith(i),
      s.localName === "astro-island" &&
        ue(i) &&
        !fe(i, s) &&
        (i.setAttribute("ssr", ""),
        i.setAttribute("props", s.getAttribute("props"))));
  j(e);
}
function j(e) {
  e.querySelectorAll("template[shadowrootmode]").forEach((t) => {
    ...
    if ((n === "closed" || n === "open") && o instanceof HTMLElement) {
      ...
      const r = o.attachShadow({ mode: n });
      (r.appendChild(t.content), t.remove(), j(r));
    }
  });
}
const ce = () => {
  ...
  le = (e, t) => {
    ...
    if (e.tagName === "STYLE" && e.textContent) {
      const r = t.head.querySelectorAll("style");
      for (const i of r) if (i.textContent === e.textContent) return i;
    }
    if (e.matches("link[rel=preload][as=font]")) {
      const r = e.getAttribute("href");
      return t.head.querySelector(`link[rel=preload][as=font][href="${r}"]`);
    }
    return null;
  },
  ue = (e) => {
  ...
  fe = (e, t) => e.getAttribute("props") === t.getAttribute("props"),
  de = (e) => {
    (re(e), ie(e), se(e));
    const t = ce();
    (ae(e.body, document.body), t());
  },
  P = (e) => document.dispatchEvent(new Event(e)),
  _ = () => P("astro:page-load");
class K extends Event {
  ...
  newDocument;
  signal;
  constructor(t, n, o, r, i, s, l, c, u, h) {
    (super(t, n),
      (this.from = o),
      (this.to = r),
      (this.direction = i),
...
async function pe(e, t, n, o, r, i, s, l, c) {
  const u = new me(e, t, n, o, r, i, window.document, s, l, c);
  return (
    document.dispatchEvent(u) &&
      (await u.loader(),
      u.defaultPrevented ||
        (P("astro:after-preparation"),
        u.navigationType !== "traverse" && H({ scrollX, scrollY }))),
    u
  );
}
const H = (e) => {
...
const M = !!document.startViewTransition,
  ...
  G = (e, t) => e.pathname === t.pathname && e.search === t.search;
let d, g, S;
const ge = () => {
  ...
  O = "data-astro-transition-persist",
  F = "data-astro-transition",
...
history.state
  ...
  : I() &&
    (history.replaceState({ index: v, scrollX, scrollY }, ""),
    (history.scrollRestoration = "manual"));
async function ye(e, t) {
  try {
    const n = new Headers(t?.headers);
    for (const [l, c] of Object.entries(V)) n.set(l, c);
    const o = await fetch(e, { ...t, headers: n }),
...
const J = (e, t, n, o, r) => {
  ...
  if (e.href !== location.href && !r)
    if (n.history === "replace") {
      ...
      history.pushState(
        { ...n.state, index: ++v, scrollX: 0, scrollY: 0 },
        "",
        e.href,
      );
  if (
    ((document.title = s),
    (S = e),
    i || (scrollTo({ left: 0, top: 0, behavior: "instant" }), (l = !0)),
    r)
  )
    ...
    } else l || scrollTo({ left: 0, top: 0, behavior: "instant" });
    history.scrollRestoration = "manual";
  }
};
function ve(e) {
  ...
  for (const n of e.querySelectorAll("head link[rel=stylesheet]"))
    if (
      !document.querySelector(
        `[${O}="${n.getAttribute(O)}"], link[rel=stylesheet][href="${n.getAttribute("href")}"]`,
      )
    ) {
...
async function q(e, t, n, o, r) {
  async function i(u) {
    ...
    const f = document.getAnimations();
    document.documentElement.setAttribute(x, u);
    const w = document.getAnimations().filter((p) => !f.includes(p) && !h(p));
    return Promise.allSettled(w.map((p) => p.finished));
  }
  const s = async () => {
    ...
    l = document.title,
    c = await we(e, n.viewTransition, s);
  (J(c.to, c.from, t, l, o),
    P("astro:after-swap"),
    r === "animate" && !n.transitionSkipped && !c.signal.aborted
...
async function Q(e, t, n, o, r, i = !1) {
  const s = Te();
  ...
  if (
    (l !== "traverse" && H({ scrollX, scrollY }),
    G(t, n) &&
      !o.formData &&
      ((e !== "back" && n.hash) || (e === "back" && t.hash)))
  ) {
    (J(n, t, o, document.title, r), s === d && (d = void 0));
    return;
  }
  const c = await pe(
    t,
  ...
  if (c.defaultPrevented || c.signal.aborted) {
    (s === d && (d = void 0), c.signal.aborted || (location.href = n.href));
    return;
  }
  async function u(a) {
    const w = a.to.href,
    ...
    if (a.formData) {
      ...
      p.body =
        t !== void 0 &&
        Reflect.get(HTMLFormElement.prototype, "attributes", y).getNamedItem(
          "enctype",
        )?.value === "application/x-www-form-urlencoded"
          ? new URLSearchParams(a.formData)
          : a.formData;
    }
    const m = await ye(w, p);
    if (m === null) {
      a.preventDefault();
      return;
    }
    ...
    if (
      ((Y ??= new DOMParser()),
      (a.newDocument = Y.parseFromString(m.html, m.mediaType)),
      a.newDocument.querySelectorAll("noscript").forEach((y) => y.remove()),
      !a.newDocument.querySelector('[name="astro-view-transitions-enabled"]') &&
        !a.formData)
    ) {
      a.preventDefault();
      return;
    }
    const k = ve(a.newDocument);
    k.length && !a.signal.aborted && (await Promise.all(k));
  }
  async function h() {
  ...
  if ((document.documentElement.setAttribute(F, c.direction), M && !i))
    f.viewTransition = document.startViewTransition(
      async () => await q(c, o, f, r),
    );
  else {
    const a = (async () => {
      (await Promise.resolve(), await q(c, o, f, r, i ? "swap" : z()));
    })();
    f.viewTransition = {
      updateCallbackDone: a,
      ready: a,
      finished: new Promise((w) => (f.viewTransitionFinished = w)),
      skipTransition: () => {
        ((f.transitionSkipped = !0),
          document.documentElement.removeAttribute(x));
      },
      types: new Set(),
    };
  }
  (f.viewTransition?.updateCallbackDone.finally(async () => {
    (await be(), _(), ge());
  }),
    f.viewTransition?.finished.finally(() => {
      ((f.viewTransition = void 0),
        f === g && (g = void 0),
        s === d && (d = void 0),
        document.documentElement.removeAttribute(F),
        document.documentElement.removeAttribute(x));
    }));
  try {
    await f.viewTransition?.updateCallbackDone;
  } catch (a) {
    const w = a;
...
async function B(e, t) {
  await Q("forward", S, new URL(e, location.href), t ?? {});
}
function Ae(e) {
  if (!I() && e.state) {
    location.reload();
  ...
  const t = history.state,
    n = t.index,
    o = n > v ? "forward" : "back";
  ((v = n), Q(o, S, new URL(location.href), {}, t, e.hasUAVisualTransition));
}
const X = () => {
  history.state &&
...
function Se() {
  for (const e of ["touchstart", "mousedown"])
    document.addEventListener(
      e,
      (t) => {
        T(t.target, "tap") && L(t.target.href, { ignoreSlowConnection: !0 });
      },
      { passive: !0 },
    );
}
...
function ke() {
  ...
  C(() => {
    for (const t of document.getElementsByTagName("a"))
      E.has(t) || (T(t, "viewport") && (E.add(t), (e ??= Re()), e.observe(t)));
  });
}
function Re() {
  const e = new WeakMap();
      ...
      o.isIntersecting
        ? (i && clearTimeout(i),
          e.set(
            r,
            setTimeout(() => {
              (n.unobserve(r), e.delete(r), L(r.href));
            }, 300),
          ))
...
function L(e, t) {
  ...
  if (De(e, n))
    if (
      (Z.add(e), document.createElement("link").relList?.supports?.("prefetch"))
    ) {
      const o = document.createElement("link");
      ((o.rel = "prefetch"),
      ...
      const o = new Headers();
      for (const [r, i] of Object.entries(V)) o.set(r, i);
      fetch(e, { priority: "low", headers: o });
    }
}
function De(e, t) {
  if (!navigator.onLine || (!t && te())) return !1;
  try {
...
(M || Pe() !== "none") &&
  ...
  document.addEventListener("submit", (e) => {
    ...
    const n = e.submitter,
      o = n && n === A;
    if (((A = null), t.tagName !== "FORM" || e.defaultPrevented || U(t) || o))
      return;
    const r = t,
      i = new FormData(r, n),
      s = typeof r.action == "string" ? r.action : r.getAttribute("action"),
      l = typeof r.method == "string" ? r.method : r.getAttribute("method");
    let c = n?.getAttribute("formaction") ?? s ?? location.pathname;
    const u = n?.getAttribute("formmethod") ?? l ?? "get";
    if (u === "dialog" || location.origin !== new URL(c, location.href).origin)
      return;
    const h = { sourceElement: n ?? r };
    if (u === "get") {
      const f = new URLSearchParams(i),
        a = new URL(c);
      ((a.search = f.toString()), (c = a.toString()));
    } else h.formData = i;
    (e.preventDefault(), B(c, h));
  }),
  Ee({ prefetchAll: !0 }));
