...
import { DEFAULT_CURSOR_MODEL, DEFAULT_EXPLAIN_AGENT_TIMEOUT_MS } from "./constants";
...
>() {
  static readonly Live = Layer.sync(AppConfig, () => {
    const explainRateLimit = Number.isFinite(Number(process.env.EXPLAIN_RATE_LIMIT))
      ...
      : 12;
    const explainRateWindowMs = Number.isFinite(Number(process.env.EXPLAIN_RATE_WINDOW_MS))
      ? Math.max(60_000, Number(process.env.EXPLAIN_RATE_WINDOW_MS))
      : 15 * 60 * 1000;
    const suggestRateLimit = Number.isFinite(Number(process.env.SUGGEST_RATE_LIMIT))
      ? Math.max(5, Number(process.env.SUGGEST_RATE_LIMIT))
      : 20;
    const suggestRateWindowMs = Number.isFinite(Number(process.env.SUGGEST_RATE_WINDOW_MS))
  ...
  static readonly explainEnabled = Effect.map(AppConfig, (config) => Boolean(config.cursorApiKey));

  static readonly suggestionsEnabled = Effect.map(AppConfig, (config) => {
    const parts = config.githubRepo.split("/").filter(Boolean);
    return Boolean(config.githubToken) && parts.length === 2 && !config.githubRepo.includes("..");
  });
}
