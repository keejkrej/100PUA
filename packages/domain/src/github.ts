...
function parseRepo(slug: string): { owner: string; repo: string } | null {
  ...
  if (!/^[-\w.\u00AA-\uFFA0]+$/.test(owner) || !/^[-_\w.\u00AA-\uFFA0]+$/.test(repo)) return null;
  return { owner, repo };
}
...
export function validateAndBuildIssue(
  ...
  if (!topicSlug || topicSlug.length > LEN.topicSlug || topicSlug.includes("..")) return null;
...
export class GithubIssuesService extends Context.Tag("@100pua/GithubIssuesService")<
  GithubIssuesService,
  {
        ...
        createSuggestionIssue: (payload) =>
          Effect.gen(function* () {
            ...
            if (issueResponse.status < 200 || issueResponse.status >= 300) {
              ...
              return yield* Effect.fail(new Error(`GitHub API ${issueResponse.status}: ${msg}`));
            }
            ...
            const htmlUrl =
              ...
              typeof (issueData as { html_url?: unknown }).html_url === "string"
                ...
                : undefined;
            const numberRaw = (issueData as { number?: number | string } | null)?.number;
            const number =
            ...
            return {
              issueUrl: htmlUrl,
              issueNumber: typeof number === "number" && !Number.isNaN(number) ? number : undefined,
            };
          }),
      };
    }),
...
export function repoConfigured(config: Context.Tag.Service<typeof AppConfig>): boolean {
