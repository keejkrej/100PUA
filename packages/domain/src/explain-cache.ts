...
export class ExplainCacheService extends Context.Tag(
  '@100pua/ExplainCacheService',
)<ExplainCacheService, ExplainDiskCache>() {
...
function explainCacheDirectory(
  projectRoot: string,
  customDir: string,
): string {
  return customDir.length > 0
    ? customDir
    : path.join(projectRoot, 'cache', 'explain');
}
...
function createExplainCache(
  ...
  async function prune(d: string, ttlMs: number): Promise<void> {
    if (ttlMs <= 0) return;
    const entries =
      await fsp.readdir(d, { withFileTypes: true }).catch(() => []);
    const now = Date.now();
