...
import { ExplainFailed, MisconfiguredServer, RateLimitExceeded, UnknownPrompt } from "./errors";
...
export const explainPrompt = (
  ...
  ExplainResult,
  MisconfiguredServer | UnknownPrompt | RateLimitExceeded | ExplainFailed | MisconfiguredServer,
  AppConfig | PromptIndexService | ExplainCacheService | ExplainAgentService | RateLimitService
> =>
  Effect.gen(function* () {
    ...
    const cachedHit = yield* cache.get(payload.slug, payload.promptId, contentKey, variantKey);
    ...
    if (!brokerPromise) {
      ...
      const timeoutId = setTimeout(() => abortController.abort(), config.explainTimeoutMs);
      ...
      brokerPromise = Effect.runPromise(
        agent.run(fullPromptText, abortController).pipe(
          Effect.tap((out) =>
            out.ok
              ? cache
                  .set(payload.slug, payload.promptId, contentKey, variantKey, out.text)
                  .pipe(
                    Effect.catchAll((e) => Effect.sync(() => console.error("[explain-cache]", e))),
                  )
              : Effect.void,
          ),
          Effect.ensuring(
      ...
      const clientMsg =
        out.error === "timeout_or_aborted"
          ...
          : typeof out.error === "string"
            ? out.error
            : "generation_failed";
      return yield* Effect.fail(new ExplainFailed({ error: "explain_failed", message: clientMsg }));
    }
