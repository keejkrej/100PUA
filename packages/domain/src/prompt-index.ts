...
export class PromptIndexService extends Context.Tag(
  '@100pua/PromptIndexService',
)<PromptIndexService, PromptIndex | null>() {
...
export function loadPromptIndex(projectRoot: string): PromptIndex | null {
  try {
    ...
    console.warn(
      '[explain-prompt] missing data/prompt-index.json - run `bun run build`',
    );
    return null;
  }
}
...
export function buildFullExplainPrompt(row: PromptRow): string {
  const chatQuery =
    typeof row.chatQuery === 'string' ? row.chatQuery.trim() : '';
